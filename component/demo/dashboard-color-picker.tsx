...
const COLOR_CHANGE_EVENT = "dashboard-color-change";

function isDashboardColor(value: string | null): value is DashboardColor {
  return colorOptions.some((option) => option.id === value);
}

function getStoredColor(): DashboardColor {
  const savedColor = localStorage.getItem(STORAGE_KEY);
  return isDashboardColor(savedColor) ? savedColor : "blue";
}

function subscribeToColorChange(onStoreChange: () => void) {
  window.addEventListener("storage", onStoreChange);
  window.addEventListener(COLOR_CHANGE_EVENT, onStoreChange);

  return () => {
    window.removeEventListener("storage", onStoreChange);
    window.removeEventListener(COLOR_CHANGE_EVENT, onStoreChange);
  };
}
