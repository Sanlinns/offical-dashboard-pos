...
function createDemoSales(): Sale[] {
  ...
  // Keep enough history for Today / Yesterday / 7 days / Month
  // and for the previous-period growth comparison.
  for (let dayOffset = 45; dayOffset >= 0; dayOffset--) {
    ...
    const weekendBoost = dayOfWeek === 0 || dayOfWeek === 6 ? 1.28 : 1;
    const trendBoost = 1 + (45 - dayOffset) * 0.004;

    const hours = [8, 9, 11, 12, 13, 15, 17, 18, 19, 20];

    hours.forEach((hour, index) => {
      const count =
        1 +
        ((dayOffset * 7 + hour * 3 + index) % 3);

      for (let orderIndex = 0; orderIndex < count; orderIndex++) {
        const created = new Date(base);
        created.setHours(
          hour,
          (dayOffset * 11 + index * 13 + orderIndex * 17) % 60,
          0,
          0,
        );

        // Avoid demo records in the future when today is still in progress.
        if (created.getTime() > now.getTime()) continue;

        const lunchDinnerBoost =
          hour >= 11 && hour <= 13
            ? 1.22
            : hour >= 17 && hour <= 20
              ? 1.38
        ...
        const baseAmount =
          900 +
          ((dayOffset * 173 + hour * 97 + orderIndex * 419) % 4200);
        ...
        rows.push({
          id: `DEMO-${dayOffset}-${hour}-${orderIndex}`,
...
const DEMO_SALES = createDemoSales();
...
function SalesAnalyticsDashboardContent() {
  ...
  // Follow the global app/sidebar light-dark mode.
  // MutationObserver also catches sidebar implementations that directly
  // toggle the "dark" class on <html>.
  const [theme, setAnalyticsTheme] = useState<ThemeMode>("night");
  ...
  useEffect(() => {
    ...
    const syncTheme = () => {
      ...
      const nextTheme: ThemeMode =
        resolvedTheme === "dark" || hasDarkClass ? "night" : "day";

      setAnalyticsTheme(nextTheme);
    };
  ...
  function toggleTheme() {
    ...
    setAnalyticsTheme(next === "dark" ? "night" : "day");
  }
      ...
      <div className="relative mx-auto w-full max-w-[1920px] space-y-5">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`text-lg font-black tracking-tight ${t.pageText}`}>Sales Analytics Demo</h1>
              <p className={`text-[11px] ${t.mutedText}`}>
                {`${filteredSales.length} / ${sales.length} demo receipts · ${dateRangeLabel}`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1.5">
              ...
              <span className="text-xs font-semibold text-blue-500">Demo data</span>
            </div>
            ...
            <button
              ...
              aria-label="Reset demo analytics data"
            >
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: 10 }}>
                  <defs>
                    <linearGradient id="gradThis" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--color-blue-600)" stopOpacity={0.4} />
                      <stop offset="95%" stopColor="var(--color-blue-400)" stopOpacity={0} />
                    </linearGradient>
                  ...
                  <Area dataKey="thisWeek" type="monotone" stroke="var(--color-blue-600)" fill="url(#gradThis)" strokeWidth={2.5} filter="url(#glow)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ...
          >
            <div className={`rounded-2xl border ${t.panelBorder} ${t.panelBg} p-5 shadow-sm backdrop-blur-xl`}>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {[
                    { label: "Revenue", value: money(analytics.thisWeekRevenue), color: "var(--color-blue-600)" },
            ...
            <div
              className={`relative overflow-hidden rounded-2xl border p-5 ${isDay ? "border-blue-200 shadow-sm" : "border-blue-500/20"}`}
              style={{
                background: isDay
                  ? "linear-gradient(135deg, rgba(255,255,255,0.94) 0%, color-mix(in srgb, var(--color-blue-100) 82%, white) 100%)"
                  : "linear-gradient(135deg, color-mix(in srgb, var(--color-blue-400) 10%, transparent) 0%, color-mix(in srgb, var(--color-blue-600) 5%, transparent) 100%)",
              }}
            >
              <div className="pointer-events-none absolute right-0 top-0 h-32 w-32 opacity-10 blur-3xl" style={{ background: "var(--color-blue-400)" }} />
              <div className="relative">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-blue-500/70">Week-over-Week</p>
        ...
        >
          Demo data only · No API connection · Drag KPI cards to reorder · Day / Night theme · Color picker ready · Hourly heatmap
        </motion.p>
      </div>
    </div>
  );
